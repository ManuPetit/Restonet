<?php
	...
	if (isset($_POST['mdpmod']))
	{
		...
		if (!preg_match('/^[a-zA-Z0-9]{5,20}$/i',trim($_POST['passa'])))
		{
			...
			if (Administrateur::CheckAdminMDP(trim($_POST['passa']),$_SESSION['userid']) == false)
			{
				...
				if (!preg_match('/^[a-zA-Z0-9]{5,20}$/i',trim($_POST['passb'])))
				{
					...
					if ((trim($_POST['passb'])) != (trim($_POST['passc'])))
					{
						...
						Administrateur::UpdateAdminMDP(trim($_POST['passb']),$_SESSION['userid']);
...
<form action="prof_mdp_form.php" method="post"  accept-charset="utf-8">
	...
	<tr valign="top">
		...
		<td><img src="../images/css/greeninfo.jpg" width="16" height="16" border="0" class="adminToolTip" title="<b>Mot de passe:</b><br />Votre mot de passe ne peut contenir que des lettres (minuscules et/ou majuscules) et/ou des chiffres.<br /><br />Votre mot de passe doit faire entre 5 et 20 caractères." />&nbsp;';
